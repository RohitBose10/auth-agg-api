...
class CategoryController {
  async addCategory(req, res) {
    try {
      ...
      return res
        .status(201)
        .json({ message: "Category created successfully.", data: { name } });
    } catch (error) {
      ...
      const categories = await Category.aggregate([
        {
          $lookup: {
            ...
            let: { catId: "$_id" }, // Passing categoryId to the pipeline
            pipeline: [
              {
                $match: {
                  $expr: { $eq: ["$categoryId", "$$catId"] }, // Matching categoryId in products with the category _id
                },
              },
              {
                $project: {
